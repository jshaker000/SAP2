...
The `assembler.rb` and `Instruction_Decoder.v` are synced to a common source, `opcodes.rb`, automatically - the former directly sources it and parses it dynamically, while
the latter is autogenerated via ERB from the same. Note that if `opcodes.rb` changes, re-assembly is required.

There is a generated ASCII reference table of the final opcodes in `OPCODES.txt`
...
You will also need `ruby` installed to run the assembler and ERB installed to generate `Instruction_Decoder.v`.
